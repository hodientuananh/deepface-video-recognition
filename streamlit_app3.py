...
for idx, Image in enumerate(Images):
    ...
    with columns_imgs[idx]:
        st.image(Image)
        st.checkbox(lst_images[idx].split('.')[0], key=str_key, value=False, on_change=lambda idx=idx: uncheck_others(idx))
def uncheck_others(selected_idx):
...
if compare_bw_face_and_em == False:
    ...
    for character_image_index in range(len(lst_images)):
        ...
        if chk_key in st.session_state and st.session_state[chk_key]:   
            ...
            character_result_dict = shot.get_sorted_shots_per_character()
...
if not compare_bw_face_and_em:
    ...
    lst_predict = shot.get_result_per_face_det_model_emb(topK=topK)
    ...
    sorted_chart_data = chart_data.sort_values(by="Average Precision", ascending=False)
    ...
    st.table(sorted_chart_data)
    sorted_chart_data.to_csv(evaluation_file, sep=',', encoding='utf-8', index=False, header=True)
